...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state: Any):
        ...
        """
        # We reach the goal if we explored all four corners
        return all(state[1])
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hits_wall:
                ...
                corner_states = list(state[1])
                if next_state in self.corners:
                    i = self.corners.index(next_state)
                    corner_states[i] = True
                cost = 1
                successors.append(
                    ((next_state, tuple(corner_states)), action, cost))
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    for i, corner_reached in enumerate(corner_states):
    ...
    # Try out every possible to path to reach all corners
    for path in itertools.permutations(corners_to_visit):
        distance = 0
        cur_pos = state[0]
        for corner in path:
            distance += util.manhattanDistance(cur_pos, corner)
            cur_pos = corner
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    farthest, nearest = None, None

    # Using `mazeDistance` to compute distance is slower
    # but gives a more accurate estimate

    for food_pos in food_list:
        key = (position, food_pos) if position[0] < food_pos[0] \
            else (food_pos, position)

        # Save the maze distance between two points
        # to avoid redundant computation
        if key not in problem.heuristicInfo:
            problem.heuristicInfo[key] = mazeDistance(
                position, food_pos, problem.startingGameState)
    ...
    return min_distance + mazeDistance(nearest, farthest, problem.startingGameState)
