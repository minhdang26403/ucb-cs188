...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(
        ...
        for elimination_variable in eliminationOrder:
            unjoined_factors, joined_factor = joinFactorsByVariable(
                current_factors_list, elimination_variable
            )
            current_factors_list = unjoined_factors
            if len(joined_factor.unconditionedVariables()) > 1:
                current_factors_list.append(
                    eliminate(joined_factor, elimination_variable)
                )
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        accumulated_prob = 0
        for key in self:
            if accumulated_prob + self[key] > random_value:
                return key
            accumulated_prob += self[key]
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for old_pos in self.allPositions:
            next_pos_dist = self.getPositionDistribution(gameState, old_pos)
            for next_pos, prob in next_pos_dist.items():
                updated_beliefs[next_pos] += current_beliefs[old_pos] * prob
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for pos_index in range(self.numParticles):
            self.particles.append(self.legalPositions[pos_index % num_legal_positions])
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            particle_weights[particle] += self.getObservationProb(
        ...
        if particle_weights.total() == 0:
            ...
            self.particles = [
                particle_weights.sample() for _ in range(self.numParticles)
            ]
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = [
            self.getPositionDistribution(gameState, old_pos).sample()
